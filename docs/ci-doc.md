...
| ✅ | Artifact-size budgets | Custom shell gate | Pre-commit + required CI | Go binary (32 MiB, re-baselined from 16 MiB when the CEL router landed), frontend JavaScript (192 KiB, re-baselined from 160 KiB when draw.io style arrows added drag handles, an output menu and arrow selection, deletion and reconnection, and the status bar gained agent health logos with inline brand marks, from 140 KiB when the editor redesign added themes, block deletion, palette explanations and a problems panel, from 110 KiB when executable blocks and their editors landed, and from 100 KiB when the SVG edge layer landed), and frontend CSS (50 KiB) have explicit limits |
