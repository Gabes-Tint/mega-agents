...
| 🟡 | Compiled-application smoke test | Shell, curl | Required CI only | Not green because it probes the real binary and embedded page over HTTP, and its `runs` command against an empty Mega Agents home, but does not exercise them in a browser |
| ✅ | Browser smoke tests | Playwright | Required CI only | Real-mouse drag-and-drop specs (palette drop, node move, scrolled-canvas placement, dialog dismissal) and run flows against the real Go backend (Git actions in temporary clones and their recorded logs, served on dedicated ports) run in the required Verify job; jsdom cannot exercise the HTML5 drag pipeline, so the browser suite covers what component tests cannot. A `make e2e` convenience target runs the same specs locally without touching the pre-commit budget |
...
| ✅ | Artifact-size budgets | Custom shell gate | Pre-commit + required CI | Go binary (32 MiB, re-baselined from 16 MiB when the CEL router landed), frontend JavaScript for the first paint (196 KiB, measured as the scripts `index.html` names itself rather than every file in the assets folder: CodeMirror moved into a chunk fetched when a property field first shows an editor, which took the entry chunk from 549933 to 197930 bytes, so the limit came down from 544 KiB to the snug step above that — near the 192 KiB it stood at before the editors landed; a field is a plain text box carrying the keystrokes, the caret and the focus until its chunk arrives), frontend JavaScript across every chunk (548 KiB, the total this gate used to check on its own; re-baselined from 544 KiB when the workflows page — where a workflow is put on a cron schedule and shown the runs it has coming — took the bundle to 559984 bytes: it is a page of its own fetched only when opened, so the first paint pays nothing for it and its weight lands here; splitting the editor out before that moved weight rather than removing it and added 1901 bytes of chunk boilerplate, taking 549933 bytes to 551834, which 544 KiB still covered, so a lazily fetched chunk still cannot grow unwatched; re-baselined from 528 KiB when the breakpoint editor took the bundle to 545874 bytes — the breakpoint and paused marks a block carries, the panel listing every block a paused run waits before with the values that reached it, and a code editor for the prompt or command that block would run, which continue, step and skip take back to the resume endpoint; the limit had been 544 KiB once before, while canvas zoom landed, although that build measured 540437 bytes and still fitted 528 KiB, and arrow colours then took the bundle to 540414 bytes by replacing three arrowheads with one, which put it back at the snug step above, from 192 KiB when CodeMirror 6 replaced the plain textareas of the command, prompt and schema fields — chosen with the product owner over a hand-rolled highlighter for real syntax highlighting, undo, bracket matching and completion of the variables that reach a block — from 160 KiB when draw.io style arrows added drag handles, an output menu and arrow selection, deletion and reconnection, and the status bar gained agent health logos with inline brand marks, from 140 KiB when the editor redesign added themes, block deletion, palette explanations and a problems panel, from 110 KiB when executable blocks and their editors landed, and from 100 KiB when the SVG edge layer landed), and frontend CSS (50 KiB) have explicit limits |
